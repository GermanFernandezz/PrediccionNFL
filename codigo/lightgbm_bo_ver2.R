...
bo_lgb <- makeParamSet(
  ...
  makeIntegerParam("num_leaves", lower = 15L, upper = 50L),
  makeIntegerParam("min_data_in_leaf", lower = 5L, upper = 40L),
  makeIntegerParam("max_depth", lower = 3L, upper = 9L)
...
#optimizacion para cada uno de los equipos
for (equipo in equipos) {
  setwd("D:/MaestriaDataMining/tallerdetesisi/nfl-big-data-bowl-2025/equipos_preprocesados")
  archivo <- paste0("plays_team_", equipo, ".csv")
  ...
  for (modelo in 1:as.numeric(length(modelos))) {
    ...
    kbayesiana <- paste0("resultados_bo_",nombre, ".RDATA")
    ...
    setwd("D:/MaestriaDataMining/tallerdetesisi/nfl-big-data-bowl-2025/equipos_preprocesados/optimizaciones/bayesiana")
