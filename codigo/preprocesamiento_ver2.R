...
equipos <- unique(games$homeTeamAbbr)
...
for (equipo in equipos) {
  ...
  numerito <- as.numeric(dim(plays_team)[2]-2)
  plays_team <- plays_team[ ,c(1,4,5,6,10,15,19,20,60,61,62,64:as.numeric(dim(plays_team)[2]),45)]
  plays_team <- na.omit(plays_team)
  # paso a character para luego poder pasar a dummies, lo necesito para redes neuronales
  ...
  write.csv(plays_team, paste0("plays_team_",equipo,".csv"), row.names = FALSE)
  write.csv(plays_team_red, paste0("plays_team_red_",equipo,".csv"),row.names = FALSE)
}

setwd("D:/MaestriaDataMining/tallerdetesisi/nfl-big-data-bowl-2025/equipos_preprocesados")
write.csv(equipos, "equipos.csv", row.names = FALSE)
